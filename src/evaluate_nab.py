...
from data_loader_nab import prepare_nab_for_anomaly_detection
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("results/lstm_autoencoder_nab.pt", map_location=device))
    model.to(device)
    ...
    np.save("results/nab_train_errors.npy", train_errors)
    np.save("results/nab_test_errors.npy", test_errors)

    save_score_plot(
        test_errors,
        test_labels,
        thr_fixed,
        "figures/nab_scores_fixed_threshold.png",
        "NAB Anomaly Scores with Fixed Threshold",
    )

    thr_90 = percentile_threshold(train_errors, q=90.0)
    save_score_plot(
        test_errors,
        test_labels,
        thr_90,
        "figures/nab_scores_percentile90.png",
        "NAB Anomaly Scores with 90th Percentile Threshold",
    )
    ...
    save_dynamic_score_plot(
        ...
        test_labels,
        rolling_thr,
        "figures/nab_scores_rolling_threshold.png",
        "NAB Anomaly Scores with Rolling Threshold",
    )

    with open("results/nab_metrics_summary.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print("\nSaved:")
    print("- results/nab_train_errors.npy")
    print("- results/nab_test_errors.npy")
    print("- results/nab_metrics_summary.csv")
    print("- figures/nab_scores_fixed_threshold.png")
    print("- figures/nab_scores_percentile90.png")
    print("- figures/nab_scores_rolling_threshold.png")